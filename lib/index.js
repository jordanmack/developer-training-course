...
function addDefaultCellDeps(skeleton)
{
	return skeleton.update("cellDeps", (cellDeps)=>cellDeps.push(locateCellDep({code_hash: DEFAULT_LOCK_HASH, hash_type: "type"})));
}
...
function describeTransaction(transaction, options)
{
	...
	for(const input of transaction.inputs)
	{
		let cell =
		{
			...
			capacityCkbytes: formattedNumber((Number(hexToInt(input.cell_output.capacity)) / 100_000_000), 2) + " CKBytes",
	...
	for(const output of transaction.outputs)
	{
		let cell =
		{
			...
			capacityCkbytes: formattedNumber((Number(hexToInt(output.cell_output.capacity)) / 100_000_000), 2) + " CKBytes",
...
async function waitForConfirmation(nodeUrl, txid, updateProgress=((_status)=>{}), options={timeoutMs: 300_000, recheckMs: 500, throwOnNotFound: true})
{
	return new Promise(async (resolve, reject) =>
	{
		let timedOut = false;
